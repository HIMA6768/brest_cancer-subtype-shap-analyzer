...
st.set_page_config(
    page_title="Breast Cancer Subtype Analysis Dashboard",
    layout="wide"
)
...
st.markdown("""
    <style>
        ...
        h1 { color: #00ffcc; font-weight: 800; text-shadow: 0px 0px 10px rgba(0,255,204,0.3); }
        h3, h4 { color: #ffffff; }
        .glass-card {
        ...
        .summary-card {
            background: linear-gradient(135deg, rgba(31,41,61,0.7) 0%, rgba(17,24,39,0.8) 100%);
...
st.markdown("<h1 style='text-align: center;'>🧬 Breast Cancer Subtype & SHAP Intelligence Dashboard</h1>", unsafe_allow_html=True)
st.markdown("---")

left_col, center_col, right_col = st.columns([1.2, 3.2, 1.1])
...
# Left Panel: Navbar & External Cloud Setup Form
with left_col:
    st.markdown("### 🧭 Navbar")
    st.markdown("---")
    st.markdown("👤 **creator: Himadri**")
    st.markdown("---")
...
# Center Panel: Main Content & Analysis Panel
with center_col:
    st.markdown("### 🔬 Patient Analysis Panel")
    st.markdown("---")

    # Error message box display if any exception occurs during external connection
    if st.session_state.db_error_msg:
        st.error(st.session_state.db_error_msg)
        st.session_state.db_error_msg = None  # Clear message after showing once
    ...
    if analyze_btn and fetched_data:
        with st.spinner("Running model inference and calculating SHAP values..."):
            ...
            with res_col1:
                ...
                st.markdown(f"<div class='metric-card'><h2 style='color: #00ffcc; margin:0;'><b>{str(subtype).upper()}</b></h2></div>", unsafe_allow_html=True)
            ...
            with res_col2:
                ...
                for sub_name, prob_val in allclassprobabilities.items():
                    ...
                    with col_name:
                        st.markdown(f"<p style='margin:0; font-weight:600; color:#e5e7eb;'>{sub_name}</p>", unsafe_allow_html=True)
                    with col_bar:
            ...
            fig_force = shap.plots.force(base_values[class_idx], shap_values_array[-1, :, class_idx], data_values, feature_names=features, matplotlib=True, show=False)
            st.pyplot(plt.gcf())
            plt.clf()
